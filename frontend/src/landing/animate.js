...
(function() {
   ...
   */
  sigma.plugins.animate = function(s, animate, animateEdge, options) {
    var o = options || {},
        id = ++_id,
    ...
    // Store initial edges:
    var startEdges = edges.map(function(edge) {
      var k;
      var res = {};
      for (k in animateEdge)
        if (k in edge)
          res[k] = edge[k];
      return res;
    }, {});
    ...
    function step() {
      ...
      if (p >= 1) {
        ...
        edges.forEach(function(edge, i) {
          for (var k in animateEdge)
            if (k in animateEdge) {
              if (k.match(/color$/))
                edge[k] = interpolateColors(
                  startEdges[i][k],
                  edge[animateEdge[k]],
                  p
                );
